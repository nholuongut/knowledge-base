...
```shell
docker run -ti nholuongut/presto
```

or

```shell
docker run -ti nholuongut/presto-dev
```

Then see the [Presto SQL](#presto-sql) section futher down.

See [DockerHub](https://hub.docker.com/u/nholuongut) for a selection of docker images - the Dockerfiles for
various Presto builds and other technologies can be found in the
[nholuongut/dockerfiles](https://github.com/nholuongut/dockerfiles) GitHub repo.
