...
while read -r renamed_from renamed_to; do
    ...
    if [ -n "$id" ]; then
        ...
        new_description="$renamed_to from nholuongut/knowledge-base repo: https://github.com/nholuongut/knowledge-base"
...
for readme in *.md; do
    ...
    if [ -n "$id" ]; then
        ...
        if [ "$gist_md5" != "$readme_md5" ]; then
            ...
            gh gist edit "$id" --filename "$readme" --desc "$readme from nholuongut/knowledge-base repo: https://github.com/nholuongut/knowledge-base" "$readme"
        else
        ...
        gh gist create --public --filename "$readme" --desc "$readme from nholuongut/knowledge-base repo: https://github.com/nholuongut/knowledge-base" "$readme"
    fi
    echo
done
